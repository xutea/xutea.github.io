import img1 from '../../assets/culture/img1.png'
import img2 from '../../assets/culture/img2.png'
import img3 from '../../assets/culture/img3.png'
import img4 from '../../assets/culture/img4.png'
export const dataList = [
    {
        img: '/public/svg/green-tea.png',
        title: '六大茶类之绿茶',
        id: '1',
        children: [{
            icon: '/public/svg/green-tea.png',
            title: '绿茶',
            id:'1-1',
            text: `绿茶是历史上最早出现的茶类，绿茶的出现距今已经有3000多年了。
            陆羽的《茶经》中已经记载有绿茶的蒸汽杀青技术。宋徽宗在《大观茶论》中对......`
        }]
    },
    {
        img: '/public/svg/red-tea.png',
        title: '六大茶类之红茶',
        id: '2',
        children:[{
            icon: '/public/svg/red-tea.png',
            title: '红茶',
            id:'2-1',
            text: '作为世界红茶鼻祖，“正山小种”迄今已有四百五十多年历史，由福建武夷山茶区的茶农发明，一直延续至今。现在印度、斯里兰卡的红茶均是由“......'
        }]
    },
    {
        img: '/public/svg/white-tea.png',
        title: '六大茶类之白茶',
        id: '3',
        children:[{
            id:'3-1',
            icon: '/public/svg/white-tea.png',
            title: '白茶',
            text: '中国茶类的划分有多种方法，根据制作方法和茶多酚氧化（发酵）程度的不同，可分为六大类(茶叶中发酵程度的轻重不是绝对的，当有小幅度的误差)......'
        }]
    },
    {
        img: '/public/svg/black-tea.png',
        title: '六大茶类之黑茶',
        id: '4',
        children:[{
            id:'4-1',
            icon: '/public/svg/black-tea.png',
            title: '黑茶',
            text: '黑茶属于后发酵茶，是我们中国特有的茶种类。通常黑茶的原料比较粗老，加之制作过程中堆积发酵的时间较长，因而叶色油黑或黑褐色，故称为黑茶。......'
        }]
    },
    {
        img: '/public/svg/qing-tea.png',
        title: '六大茶类之青茶',
        id: '5',
        children: [{
            id:'5-1',
            icon: '/public/svg/qing-tea.png',
            title: '青茶',
            text: '青茶俗称乌龙茶，属半发酵茶，是我国六大基本茶类中独具特色的一个茶类。青茶的总体品质，介于红茶与绿茶之间，兼具两种茶的品质特征，即色泽青褐，汤色黄亮，具有浓郁的花香，叶底绿叶红镶边......'
        }]
    },
    {
        img: '/public/svg/yellow-tea.png',
        title: '六大茶类之黄茶',
        id: '6',
        children: []
    },
    {
        img: '/public/svg/puer-tea.png',
        title: '普洱茶',
        id: '7',
        children: []
    },
    {
        img: '/public/svg/tea-dishes.png',
        title: '茶皿',
        id: '8',
        children: []
    },
    {
        img: '/public/svg/tea-culture.png',
        title: '茶文化',
        id: '9',
        children: [{
            icon: img1,
            title: '茶与神农（理工版）',
            id:'9-1',
            text: '“茶圣”陆羽在《茶经·六之饮》中说：“茶之为饮，发乎神农氏……”说的是茶作为一种饮料被人类所用，是从神农开始的。历代茶文化研究者......'
        },
        {
            icon: img2,
            title: '茶与神农（文史哲版）',
            id:'9-2',
            text: '“茶圣”陆羽在《茶经·六之饮》中说：“茶之为饮，发乎神农氏……”说的是茶作为一种饮料被人类所用，是从神农开始的。历代茶文化研究者......'
        }]
    },
    {
        img: '/public/svg/practical-activity.png',
        title: '我们的实践活动',
        id: '10',
        children:[{
            icon: img3,
            id:'10-1',
            title: '茗香飘万里 何须恋糖精',
            text: '中国是茶的故乡，同时也是茶文化走向世界的窗口。在中国的神话传说中，茶是神农氏尝百草后解毒的灵药；在古代时，茶是士大夫们抒发雅兴的道具......'
        },
        {
            icon: img4,
            id:'10-2',
            title: '茶与养生保健',
            text: '昨天线下活动开展后，有同学问我，喝茶到底对身体有什么好处？这毕竟是个严肃的问题，回答的内容必须要有依据。于是我查阅了一些资料，并整理如......'
        }]
    }
]